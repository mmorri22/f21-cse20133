...
INTRO_MESSAGE="Grading script for Programming Challenge 05"
...
CHECK_EXE="test -f exe/max_prod_sub"    

# Next, we check for the object files  
if test -f "objects/max_prod_sub.o"; then
...
if test -f "exe/max_prod_sub"; then
    echo "max_prod_sub Executable Exists." >> $SCRIPT_OUT
	echo "Executables must be cleaned prior to GitHub push." >> $SCRIPT_OUT
	echo "Automatic 50 point deduction, per project description" >> $SCRIPT_OUT
...
MAKE_PC05_TEST="make max_prod_sub"
MAKE_GCC="g++"
MAKE_WALL="-Wall"
MAKE_WEXTRA="-Wextra"
